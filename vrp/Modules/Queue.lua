...
Config.Priority = {
    ["steam:110000136803a96"] = 100,
}

Config.RequireSteam = false  -- steam only server
            ...
            while true do
                ...
                if not pos or not data then
                    if data and data.deferrals then 
						data.deferrals.done(Config.Language.timedout) 
					end
                    CancelEvent()
                    Queue:RemoveFromQueue(ids)
                    Queue:RemoveFromConnecting(ids)
...
exports.connectqueue:AddPriority("ip:200.9.155.188", 50)
